...
CSV = "/mnt/c/Users/JoepvanSchagen/Downloads/claude/seasonal_gap_urls_gt1_20260603.csv"
with open(CSV) as f:
    rows = list(csv.DictReader(f))
url_ids = [int(r["url_id"]) for r in rows]
assert not any(("+" in r["url"] or "=" in r["url"]) for r in rows), "found '+'/'=' url in CSV!"
print(f"seasonal gap url_ids from CSV (visits>1, no '+'/'='): {len(url_ids):,}")
