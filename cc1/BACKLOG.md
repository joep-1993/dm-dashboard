...
- [x] **Morfeem-dekking als rangschikking in `match_by_token_coverage`** (logged 2026-08-26, **SHIPPED same day**, `68621f0`). Als het **categoriewoord** van de query even goed op álle waarden van een as matcht, beslist de tiebreak op producttelling — en dat is systematisch de verkeerde waarde. `/tuin_accessoires_504073/r/kussens_voor_loungeset/` (Tuinkussens): 'kussens' matcht elke `t_tuinstoelkussen`-waarde die erop eindigt, dus wint 'Zitkussens' (3.625) van 'Loungekussens' (371), terwijl 'loungeset' het onderscheidende token is. `/speelgoed_spelletjes_395613/r/speelgoed_magneten/` (Bouwstenen): 'speelgoed' matcht `levensfase~'Babyspeelgoed'`, terwijl `type_bouwblok~'Magnetisch bouwspeelgoed'` (323 producten) béide tokens dekt. In beide gevallen deelt het beslissende token alleen een **morfeem** met de goede waarde ('lounge', 'magnet') en vergelijkt de matcher hele tokens — `_tokens_equal_modulo_morphology` komt niet verder dan meervoud/dubbele-klinker, en `_is_semantic_match` eist dat een prefix-rest een uitgang is. Vorm van de fix: morfeem-dekking als **rangschikkingsterm** (hoeveel querymorfemen dekt de waarde, hoeveel waarde-morfemen blijven onbenoemd), niet als filter — een filter zou de bestaande matches breken. Raakt het primaire pad voor elke query met 2+ tokens, dus corpus-validatie op de 998-rij-A/B vóór het landt. **DONE:** `_token_match_weight` geeft 1.0 voor het token zelf en 0,5 voor een gedeeld morfeem (`_shared_morpheme_len`: langste van gemeenschappelijke kop en staart, ≥ 5 tekens, beide tokens ≥ 6 — een gedeeld midden telt niet), en de scorer rangschikt op de som. Drie randvoorwaarden bleken nodig: bodem 1.0 per waarde (een halve match mag niet alléén winnen, want deze scorer keert direct terug uit `match_with_partial`), een herhaald token telt één keer ("platenspeler platenspeler" haalde met twee halve morfemen de bodem en liep naar `s_kasten~'Platenspelerkasten'`), en de V35-graft mag geen as toevoegen die het coverage-resultaat al zegt. 19 urls anders over de 998, A 118 → 121. Zie LEARNINGS "Het morfeem dat aan de staart van het ene woord en de kop van het andere zit".
- [ ] **Een merkwaarde die gelijk is aan het generieke productwoord glipt door de merk-guards** (logged 2026-08-26). `/tuin_accessoires_504057/r/kleine_gas_bbq_camping/` (Barbecues) kwam uit op Barbecue-accessoires `/c/merk~11938694` — een merk dat letterlijk **'BBQ'** heet, 13 producten. `brand_mention_is_distinctive` (de V39/V57-guard) vraagt of de query het merk noemt, en dat doet hij: 'bbq' staat er letterlijk. De guard kan niet zien dat het woord het productbegrip is en niet het merk. Kandidaat-regel: een `merk`-waarde afwijzen als de genormaliseerde naam gelijk is aan (of de kop is van) de naam van de maincat/subcategorie of een bekend generiek productwoord — dat is een vaste eigenschap van de facetwaarde, dus in principe éénmalig te bepalen over `facets.csv`. Losstaand: dezelfde r-url zou moeten afdalen naar het kind Gasbarbecues + `formaat_barbecue~Klein`, maar search-derived heeft daar te weinig bewijs voor (2 producten AND-match) en de facetroute komt niet tot de kind-categorie. Zie LEARNINGS 2026-08-26 en TASKS "Auto-redirects V62". **UPDATE 2026-08-26 (`2fc5f8f`):** een tweede, ándere route naar hetzelfde soort onzin is wél gedicht — `/r/camping_kooktoestel/` kreeg `merk~Campingaz` omdat de Fix-D-append op `_isb` filterde, wat "dit is een merk-as" betekent en niet "de query noemt het merk"; daar geldt nu `_spurious_brand_facet`. Het punt hier blijft staan, want dat is precies het geval waar de distinctiviteitstoets ja zégt: bij `merk~'BBQ'` staat het woord letterlijk in de query. Dat vraagt om een eigenschap van de facetwaarde (merknaam == generiek productwoord of == de categorienaam), éénmalig te bepalen over `facets.csv`.
