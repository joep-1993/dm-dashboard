...
- **Run result now renders as a table** (frontend, `renderRunResultPanel`) mirroring the preview: tiles (created/paused/skipped/errors + all, click-to-filter) + sortable table + filter + Export/Copy, instead of raw JSON. Plus a real 0-100% **run progress bar** (`_run_progress` + `GET /run/progress`, polled).
