...
**Deploy (2026-07-12):** the v24 code needs the newer library — commit `35d2def` says "upgrade google-ads 27.0.0 → **31.1.0** (default API v24)" but did NOT bump `requirements.txt` (was `>=25.1.0`; the env had 30.0.0 which tops out at v23). No tool sets an explicit `version=`, so they all float on the library's default — meaning a restart on 30.0.0 would run v24-dependent code (`feed_label` vs removed `sales_country`, required `contains_eu_political_advertising`) against a v23 default and break. Fixed: `pip install google-ads>=31.1.0` (31.1.0 supports v21–v24, default v24), bumped `requirements.txt` to `>=31.1.0`, restarted backend (pid 380764). Smoke-tested: all 8 Google-Ads modules import; health 200; **live pause/re-enable of a real campaign works under v24**; shop-changes query returns the known 5 rows for 2026-07-10. **Lesson: an API-version upgrade in code MUST bump `requirements.txt` in the same commit — the version floats on the installed library's default, so a code/lib mismatch is a restart-time landmine.**

**main brought to parity (2026-07-12):** `main` had the v24 code but not the `requirements.txt` bump (own restart-landmine), and lacked the seo-stats/DMA-exclusions work. Cherry-picked onto `main`: the requirements bump (`7d6ce75`), seo-stats transactional revenue + Per-day toggle (`c1b4949`), DMA-exclusions polish (`a08edd0`) — all applied cleanly (main had never touched those files). `origin/main` and `rurl-v45-confidence-scoring` are now **code-identical** (only `cc1/` docs differ, which live on the working branch). The live backend still runs from the working-branch tree; code is the same on both.
...
- **Old vs new revenue.** The tool computed revenue at **visit grain** from `datamart.fct_visits` as `cpc_revenue + ww_revenue` (affiliate *excluded*, plus a 0-productclick glitch guard). It now uses **Beslist's own revenue** — `bt.cpa_outclicks_transactional.click_revenue` ("onze omzet", the "berekende shop kosten / onze omzet") — which **includes affiliate commission** (CPA + cpa_cpc + t3_fallback + shoppingcart/WW + affiliate) and is attributed by **conversion date**. Filters: `actual_ind=1 AND deleted_ind=0 AND label NOT IN ('cpa_after_180_days','rejected_click')`, channel via `chan_deriv.ref_channel_derivation_stats` (`+ deleted_ind=0` on the join to avoid fan-out).
- **Two grains, merged in Python.** Visits stay visit-grain (`fct_visits`, `is_real_visit=1`); revenue is a separate query on the transactional table. `_fetch_daily` merges by (date, channel); `_fetch_channel_deltas` by channel; `_fetch_cat_deltas` by the `(maincat, cat)` name tuple (union of keys — a category can have visits but no conversions or vice-versa). Category link differs by grain: visits via `fv.dim_category_key = cat.dim_category_key`; revenue via `tac.deepest_category_id = cat.deepest_category_id` (**lossless** — verified 0 unmapped rows, mapped total == grand total). Both resolve names via `datamart.dim_category`, so the name tuple is a stable merge key.
- **Reconciliation (the whole point).** Qlik measure `T-SEO2: SEO omzet` = `SUM({<_indOmzet=1, kanaal={'SEO','Overig Kanaal'}>}omzet) + adsense`; the *pure* SEO channel via `Totale Omzet (kanalen)` broken down by `Kanaal` = **€13 412,29** for 2026-07-01. Redshift `SUM(click_revenue)` for `marketing_channel='SEO'` all-domain = **€13 300,74** → **0,8% match**. The pre-aggregated `bt.daily_standup_metrics_category.omzet_total` was ~5% high (its `affiliate_revenue` runs larger than transactional `click_revenue`), so the **transactional base table is the right reconciliation source, not the standup aggregate**. `omzet_visit` (€9 068) is a different metric (visit-attributed, for OPB) — never use it here.
- **Settling caveat.** `click_revenue` keeps settling for up to ~180 days after the conversion date, so the most recent 1–2 days undercount (same reason the Qlik number keeps moving). Kept the existing comparison offsets (visits ref vs ref-7; revenue ref-1 vs ref-8) and added a UI note.
- **Perf.** The transactional query is slow (~73s cold for a single day; the table is large). The 5-min in-process TTL cache covers repeat loads. Watch the 30/90-day ranges.
- **Per-day overview toggle.** `TABLE_COLS` was a `const` referenced in ~8 render sites. Made it `let` with `TABLE_COLS_BASE` + `TABLE_COLS_EXTRA=['dma_omzet','gsaas_omzet']`; `toggleExtraRev()` swaps it and re-renders (head/sort/foot/page), falling back to Date-sort if the active sort column is hidden. `WOW_BY_KEY` now derives from base+extra so WoW works for the extra cols (`initTable` already computed WoW for every `ORDER` key). Extra cols aren't in `HEAT_KEYS` → plain (non-heatmapped) cells, which is fine.
- **Data-driven frontend.** The chart, Per-day table, Top categories and Performance standup all read the backend's `*_omzet` / `revenue_p1/p2/delta/pct` fields, so **changing the backend's values updated every module with no frontend change** (only the toggle + caveat note were frontend edits).

## DMA Exclusions — light-yellow done-banner + collapse empty OOS table after a run (2026-07-10)

- **Banner colour.** The OOS "Done — processed N product(s) · N excluded" banner used Bootstrap `alert-success`, but the shared `/static/css/style.css` theme overrides Bootstrap alert backgrounds to grey (hence the existing `!important` on `.badge-*`). Added `.alert-done-yellow { background:#fff8e1 !important; … }` and switched the success branch of `showOosDone()` to it (failed→`alert-warning`, cancelled→`alert-info` unchanged).
- **Empty table lingering.** After an exclusion run, `excludeSelectedOos()` calls `scanOos()` to refresh — which unconditionally re-shows the table/filters/actions even when the refreshed scan returns nothing, leaving an empty "No matching rows." table under the banner. Added `hideOosResults()` (hides `#oosControls`/`#oosActions` with `display:none !important` — they use inline `!important` — and `#oosTable`) and call it after the refresh **only when `oosVisible().length === 0`**, so filter-driven empties during interactive use don't hide the filter box. A fresh "Scan OOS" reveals everything again.
...
## GSD: remove_campaign fixed + LL badge colours — 2026-07-13
- **remove_campaign FIXED** (commit `5d17b5d`): `_mutate_campaign_status` now branches — `REMOVED` uses the dedicated `op.remove` REMOVE operation; ENABLED/PAUSED keep the field-mask status update. Setting `status=REMOVED` via update is rejected (`INVALID_ENUM_VALUE`). Verified live: created a PAUSED test campaign → `remove_campaign` → status REMOVED → budget cleaned up. (Supersedes the "NEW BUG (not yet fixed)" note above.)
- **LL summary badges** (commit `f1beab6`): "errors" badge always red (`bg-danger`, was grey when 0); "skipped" badge purple (`#5e4a90`), dropping the barely-visible `bg-light` grey (per the avoid-grey-labels preference).
- **Deploy**: remove fix needs a uvicorn restart to go live; the badge change is static (live on save).
