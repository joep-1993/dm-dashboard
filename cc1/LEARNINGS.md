...
- **Phase 1 (SHIPPED, V44).** In the Fix D `else` branch, enrich the dominant same-maincat category with probe-matched facets. Both rules gated on `_keyword_bridges_value(keyword, value_name)` (keyword must literally match the facet value):
  ...
  - **non-brand** facet: append ONLY when `name_link` = the dominant category NAME is itself lexically on-topic (`_keyword_bridges_value(keyword, dom_cat_name)`) → `inklapbaar droogrek muur` → `…505130_505247/c/bevestiging_rekken~19275898~~o_droogrek~23591184`. This guard stops `waxinelicht groot` bolting `f_woonacc~Groot` onto the OFF-topic `Gedenkartikelen` (name_link False) — it stays bare.
