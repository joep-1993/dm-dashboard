...
- **Auto-repair only builds a tree when there is NONE — it treats an existing *wrong* tree (single root UNIT) as "complete" and skips it.** So fixing pre-existing wrong-tree campaigns needs an explicit rebuild: remove the existing LISTING_GROUP criteria (`AdGroupCriterionOperation.remove`) then re-run `add_sub_cpr`. Ad-group renames = `mutate_ad_groups` update with `update_mask.paths=['name']` (cosmetic; doesn't touch targeting).
