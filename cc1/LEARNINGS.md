...
- **Re-enable button had no progress indicator (user report).** `reenableRecovered()` did two awaits (`/oos/recovered` check — now a bulk monitor pass — then `/oos/reenable` POST doing a Google Ads write per product) with the button static the whole time. Added a phase-aware spinner: locks the button + shows `Checking…` then `Re-enabling N…`, restored in a `finally`. Right-sized to a button loader (not a progress bar) because the endpoint is a single server-side loop, matching the existing exclude-button setup pattern. Note: the run still surfaces no per-item progress (a browser alert at the end is the only completion signal) — a streaming/SSE re-enable with a real bar is the next step if wanted.
