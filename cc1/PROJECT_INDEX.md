...
- **`backend/rurl_optimizer_v2/main_parallel_v2.py`** — category-scoped R-URLs (`/products/{maincat}[/{subcat}]/r/{kw}/`). Pipeline: **V70 kale-merk-sluiting** (query == één merknaam + geen subcat in de url → `/products/{maincat}/c/merk~{id}`, `_v70_brand_page`) → matcher (`src/matcher.py`) → V28 search-derived rescue (`src/search_derived.py`) → V29/V31 facet-probe (`src/facet_probe.py`) → reliability scorer (`src/reliability_scorer.py`) → `src/url_builder.py`. CLI: `python main_parallel_v2.py input.csv -o out.csv -c r_url --threshold 80 --multi-facet --enable-facet-probe` (run from the `rurl_optimizer_v2/` dir).
- **`backend/rurl_optimizer_v2/process_global_rurls.py`** — maincat-less "mainpage" R-URLs (`/products/r/{kw}/`, ~10.5k) that the category parser rejects. Matches the keyword across all categories; patches results back into the full dataframe so category rows are preserved. Discovery order: (0) **V70** — de hele query is één merknaam → de merk-facet op maincat-niveau, waarbij de maincat met de meeste producten van dat merk wint (score 95 vanaf 70% aandeel, anders 75); vereist >1 subcategorie en een subcat-naamguard op gelijkheid, niet op de fuzzy ≥95 (taxonomie-breed scoort `jack daniels` >95 tegen "Jacks"). (1) subcat-NAME match ≥95 → full in-subcat facet match; (1.5b) `[global_type_subcat]` — best-scoring type-facet value pinpoints a subcat, then full in-subcat match; (2) cross-type fallback; (3) low-threshold subcat name. Accepts both relative and absolute URL shapes (parser `_invalid_global` + domain-optional `GLOBAL_RURL_PATTERN`).
...
- **Doorvoeren naar productie** (`backend/rurl_results.py`, sinds `0a5658c`): leest de opgeslagen run-xlsx uit `rurl_run_output` terug als rijen, zodat élke run uit de historie doorvoerbaar is. `GET /api/rurl{,-v2}/results/{task_id}?min_score=&limit=` geeft rijen + het volledige score-histogram (de drempelselector toont daarmee aantallen per tier); `GET .../export?task_ids=a,b,c` streamt één run byte-voor-byte terug en merget vanaf twee runs met een `run`-kolom. Leest beide outputschema's (v2's `old url`/`new url`/`score` en v1's ruwe kolommen). De frontend-kaart "Push Redirects" in `frontend/rurl-optimizer.html` voedt hetzelfde `/api/redirect-tool` preview+submit-paar als Canonicals en de Redirect Generator.
- Deep design notes + gotchas: see cc1/LEARNINGS.md (2026-05-19 "V31 facet-probe", 2026-05-27 "facet-probe reachability / dom_cat semantics / two-pipeline routing", 2026-05-27 session 2 "global R-URL routing + facet-selection quality", 2026-09-02 "een score die het bewijs van de kále categorie meet" — waarom `dom_cat_share` een gefacetteerde bestemming afstraft, en 2026-09-02 "een cascade met returns halverwege" — de staart van `process_url_v2` is vijf order-gevoelige stappen en zit sinds V67 in `_finalize_redirect()`, plus de ontleding van de cross-maincat-ladder in identiteit (`_cross_maincat_base`) versus de banden van `score_search_derived`, en Joeps maat/kleur-vloer op tier C, en 2026-09-08 "een score die precies op een vloer landt" — RC5's value≡query-vloer van 90 is blind voor de categoriehelft van de /c/-H1, V69 trekt hem terug als de search-leider de sprong niet steunt, en een A/B-vlag in de parent bereikt de spawn-workers niet, en 2026-09-09 "een kale merkquery hoorde nooit in een subcategorie" — `facets.csv` kent nul maincat-niveau urls, dus de count-leader-dedup móét een subcategorie kiezen; V70 bouwt de maincat-url zelf en stelt hem vrij van de V61-pruning, en een bezitsapostrof mag bij het vouwen van merknamen geen woordgrens zijn).
