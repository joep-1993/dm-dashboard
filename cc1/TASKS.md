...
- [x] DMA+ bidcat migration — DMA campaigns are named `PLA/{bidcat}_{cl1}` (isBiddingCategory=true), NOT `PLA/{deepest_cat}_{cl1}`. Taxonomy API's subcategory summaries omit the `isBiddingCategory` flag (only top-level detail carries it), so the new `_fetch_bidcats_recursive` fetches each category individually via `GET /api/Categories/{id}` and keeps only bidcats. `requests.Session` reuse keeps the TLS handshake cost down — benchmark on Klussen: 284 API calls, 140 bidcats, ~19s. Tuple format unchanged so the `cat_ids` sheet + `process_exclusion_sheet_v2` work without downstream changes. File: `backend/dma_plus_service.py:215-291` #claude-session:2026-04-17
