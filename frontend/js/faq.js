...
let faqBatchPolling = false;

function toggleBatchMode() {
    faqBatchMode = document.getElementById('batchApiCheckbox').checked;
...
async function startBatchProcessing() {
    const numFaqs = parseInt(document.getElementById('numFaqsInput').value) || 6;
    ...
    try {
        const response = await fetch(`${API_BASE}/api/faq/batch-start?num_faqs=${numFaqs}`, { method: 'POST' });
        ...
        // Poll for status
        faqBatchPolling = true;
        while (faqBatchPolling) {
            await new Promise(r => setTimeout(r, 3000));
            try {
                const statusResp = await fetch(`${API_BASE}/api/faq/batch-status`);
                const status = await statusResp.json();
                updateBatchProgress(status);

                if (!status.active) {
                    faqBatchPolling = false;
...
function resetBatchUI() {
    document.getElementById('processBtn').disabled = faqBatchMode;
...
// Export functions
// Recent FAQ Results export: one button, the #faqExportFormat dropdown decides the
// target. Replaced three separate Export Excel / JSON / Combined buttons.
//
// "combined" is deliberately in the same list even though it is a different
// endpoint (FAQ + content_top in one workbook, not a FAQ-only export in another
// format) — from the user's side it is still "what do I want out of here".
const FAQ_EXPORT_PATHS = {
    xlsx:     '/api/faq/export/xlsx',
    json:     '/api/faq/export/json',
    combined: '/api/export/combined/xlsx',
};

function exportFaqResults() {
    const sel = document.getElementById('faqExportFormat');
    const fmt = sel ? sel.value : 'xlsx';
    const path = FAQ_EXPORT_PATHS[fmt];
    if (!path) {
...
async function cancelFaqRecheck(taskId) {
    try { await fetch(`${API_BASE}/api/recheck-skipped-urls/cancel/${taskId}`, { method: 'POST' }); } catch (e) {}
}

async function cancelFaqValidation(taskId) {
    try { await fetch(`${API_BASE}/api/faq/validate-all-links/cancel/${taskId}`, { method: 'POST' }); } catch (e) {}
}
        ...
        const poll = setInterval(async () => {
            try {
                ...
                if (data.status === 'running') {
                    ...
                    clearInterval(poll);
                    resultDiv.innerHTML = `<div class="alert alert-info"><strong>Validation cancelled.</strong><br>Validated ${(data.validated || 0).toLocaleString()} FAQs before stopping.</div>`;
                    await refreshFaqStatus();
...
async function recheckSkippedFaqUrls() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/faq/recheck-skipped-urls?parallel_workers=${workers}&batch_size=${batchSize}`, { method: 'POST' });
        const startData = await response.json();
        const taskId = startData.task_id;
            ...
            try {
                ...
                if (data.status === 'running') {
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <div class="progress flex-grow-1" style="height: 25px;"><div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: ${pct}%"></div></div>
                            <button class="btn btn-sm" style="border: 1px solid #d63031; color: #d63031;" onmouseover="this.style.background='#d63031';this.style.color='white'" onmouseout="this.style.background='transparent';this.style.color='#d63031'" onclick="cancelFaqRecheck('${taskId}')">Cancel</button>
                        </div>
                    </div>
                    <div class="alert alert-warning mb-0">
